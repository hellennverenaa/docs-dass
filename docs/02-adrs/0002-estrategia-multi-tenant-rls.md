# Diretriz: Estratégia de Isolamento Multi-Tenancy
...
* **Objetivo:** Centralizar manutenções e custos em uma única infraestrutura, simplificar migrações do TypeORM e permitir relatórios gerenciais consolidados em tempo real entre todas as unidades.

---

## 2. Diagrama de Isolamento das Unidades

O diagrama abaixo apresenta o isolamento dos dados entre as fábricas através da nossa API e do filtro automático no PostgreSQL:
...
O fluxo a seguir detalha o passo a passo da requisição desde o clique do operador até o retorno filtrado pelo banco:
...
* O valor de `unit_id` deve ser extraído sempre do token validado na nossa API, nunca confiando em dados livres enviados pelo formulário do front-end.

### 4.2. Segurança Nativa (RLS)
* As tabelas devem ter o RLS habilitado no banco (`ALTER TABLE nome_tabela ENABLE ROW LEVEL SECURITY`).
* O banco de dados bloqueia consultas que tentem ler ou alterar registros que não correspondam à unidade associada à conexão.

### 4.3. Indexação Composta
* Consultas operacionais filtram frequentemente por unidade e status.
* Índices de tabelas pesadas devem incluir a coluna `unit_id` no início da composição para evitar lentidão nas telas de listagem.